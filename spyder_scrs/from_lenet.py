...
def evaluate(X_data, y_data,drop_prob_valid):
...
with tf.Session() as sess:
    ...
    for i in range(EPOCHS):
        ...
        for offset in range(0, num_examples, BATCH_SIZE):
            ...
            if end >= num_examples:
                train_accuracy.append(sess.run(accuracy_operation, feed_dict={x: batch_x, y: batch_y,keep_prob:drop_prob_train}))
                #print("step %d " %(i))
            #######
        validation_accuracy = evaluate(X_valid, y_valid,drop_prob_valid)
        ...
        writer=tf.summary.FileWriter('./nn_logs1',sess.graph) 
